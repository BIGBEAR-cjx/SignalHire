...
| 评分项 | 仓库证据 |
|--------|----------|
| Repo 结构规范 | 根目录只保留产品/验证/测试入口；`web/`、`worker/`、`docs/`、`migrations/` 按职责拆分 |
| README 清晰 | 本文件说明产品定位、demo、架构、运行方式、项目结构和验证命令 |
| DEMO 可访问 | 生产 demo 首页和 `/app/search` 已返回 HTTP 200；详见 `docs/DEMO.md` |
| 核心功能真实完成 | 搜索、实时研究进度、反馈优化下一轮、项目下一步、分享报告、worker queue 均有代码和测试覆盖 |
| 技术逻辑完整 | `docs/ARCHITECTURE.md` 说明 web app、Insforge queue、worker、MiroMind、normalizer 和验证链路 |
| 持续开发迭代能力 | `docs/superpowers/plans/` 保留阶段计划，`docs/verification.md` 保留 smoke checks，单元测试覆盖核心 domain helpers |
...
结果：单元测试、生产构建、diff whitespace check 均通过；生产 demo 和搜索入口均返回 HTTP 200。
...
```
README.md       Product, demo, architecture, and verification entry point
PRODUCT.md      Product register, audience, purpose, and design principles
DESIGN.md       SignalHire design system and UI rules
docs/
  DEMO.md       Public demo URL, demo paths, and smoke checks
  ARCHITECTURE.md
                System architecture and data flow
  verification.md
                Local, production, worker, and live-job verification notes
  superpowers/  Implementation specs and iteration plans
web/            Next.js app (App Router + Tailwind)
  app/          App routes, public landing, auth, projects, search, reports
  components/   Shared product UI and research workspace components
  lib/          Domain helpers, i18n, Insforge access, MiroMind client
  scripts/      Schema, live-job, retry, and worker-health checks
worker/         Long-running Node worker for queued non-cached research
migrations/     Research queue reliability migration
*.test.mjs      Node test suites for domain and integration helpers
```
