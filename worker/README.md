...
| `MAIGRET_ENABLED` | 可选。设为 `1`/`true` 时在开放证据预检里启用 Maigret username 补搜；默认关闭 |
| `MAIGRET_COMMAND` | 可选。Maigret CLI 或 sidecar wrapper 命令，默认 `maigret` |
| `MAIGRET_MAX_ALIASES` / `MAIGRET_TIMEOUT_MS` / `MAIGRET_TOP_SITES` | 可选。Maigret 每次最多 alias、单 alias 超时、站点数量；默认 `3` / `90000` / `200` |
| `MAIGRET_TAGS` / `MAIGRET_EXCLUDE_TAGS` | 可选。Maigret 搜索标签与排除标签；默认 `coding,global,us` / `dating,nsfw,porn` |
| `WORKER_CONCURRENCY` | 可选。单个 worker 同时处理的任务数，最大为 `3`，默认 `3` |
...
- It claims up to three rows by changing `status` from `queued` or `retrying` to `running`.
...
- Maigret 默认关闭。若开启 `MAIGRET_ENABLED`, 需要在镜像中安装 `maigret` CLI，或把 `MAIGRET_COMMAND` 指向一个兼容的 sidecar wrapper。
...
- **认领防并发**:单个 worker 最多并发 3 个任务；每个任务仍用 `update status=running where id=? and status in ('queued','retrying')` + `.select()` 确认。
...
- **Maigret 补搜边界**:只从明确的 GitHub/Hugging Face URL 提取 username, 只写入 `open_evidence_leads`, 不把账号存在性当作已验证证据。默认排除 dating/nsfw/porn 标签, 并跳过 LinkedIn host；LinkedIn 只能通过后续合规 MCP/官方 API provider 单独接入。
