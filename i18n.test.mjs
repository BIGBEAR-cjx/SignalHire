...
test("translates research fallback error labels", () => {
  const keys = [
    ...
    ["api.error.resumeTooLarge", "文件超过 4MB，请压缩后再上传", "File is larger than 4MB. Compress it and upload again."],
    ...
    ["api.error.resumeUploadInterrupted", "文件过大或上传中断，请压缩到 4MB 以内再上传", "The file is too large or the upload was interrupted. Compress it below 4MB and upload again."],
    ...
    ["api.error.projectNotFound", "项目不存在", "Project was not found."],
    ["api.error.invalidProjectName", "name 非法", "Invalid project name."],
    ["api.error.invalidProjectBrief", "brief 非法", "Invalid project brief."],
    ["api.error.invalidProjectStatus", "status 非法", "Invalid project status."],
    ["api.error.invalidProjectColor", "color 非法", "Invalid project color."],
    ["api.error.projectUpdateUnavailable", "更新失败或项目不存在", "Update failed or project was not found."],
    ["api.error.projectDeleteUnavailable", "删除失败或项目不存在", "Delete failed or project was not found."],
...
test("translates resume upload labels", () => {
  const keys = [
    ...
    ["research.resumeUploadSupported", "支持 PDF / DOCX / TXT，单个文件 4MB 以内，不保存原文件。", "Supports PDF / DOCX / TXT up to 4MB. Original files are not stored."],
...
test("translates job lifecycle fallback labels", () => {
  const keys = [
